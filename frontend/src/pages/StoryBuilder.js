...
export default function StoryBuilder() {
  ...
  const lastId =
    draft.conflicts[draft.conflicts.length - 1]?.id;

  const firstDetail = firstId
    ? detailCache[firstId]
    : null;

  const lastDetail = lastId
    ? detailCache[lastId]
    : null;
  ...
  const allTexts = useMemo(() => {
    ...
    if (draft.a_clause) {
      texts.push(draft.a_clause.description);
    }

    if (draft.b_clause) {
      texts.push(draft.b_clause.description);
    }

    if (draft.c_clause) {
      texts.push(draft.c_clause.description);
    }

    spineDetails.forEach((d) => {
      d.permutations.forEach((p) => {
  ...
  const handleExport = () => {
    ...
    if (draft.a_clause) {
      lines.push(
        `**Beginning:** ${stripLeadArticle(
          draft.a_clause.description
        )}`
      );
    }
    ...
    if (draft.b_clause) {
      lines.push(
        `**Middle:** ${draft.b_clause.description}`
      );
    }

    if (draft.c_clause) {
      lines.push(
        `**Conclusion:** ${draft.c_clause.description}`
      );
    }
    ...
    spineDetails.forEach((d, i) => {
      const text = d.permutations
        .map((p) => p.description)
        .join(" ");

      lines.push(
        `${i + 1}. ${renderPlainEnglish(
          text,
          characters,
          alias
        )}`
    ...
    <div className="flex flex-col lg:flex-row h-screen">
      ...
      <div className="lg:w-80 border-r border-line-strong bg-surface h-full overflow-y-auto thin-scroll p-5 flex flex-col gap-5">
        <div className="flex items-center justify-between">
          <h2 className="text-xs tracking-[0.2em] uppercase text-ink-muted">
            Story Studio
          </h2>
        ...
        <div className="space-y-3">
          <ClausePicker
            label="Beginning (A)"
            clauses={clauses.a || []}
            selected={draft.a_clause}
            onSelect={(c) =>
              setField("a_clause", c)
            }
            testIdPrefix="a_clause"
          />

          <ClausePicker
            label="Middle (B)"
            clauses={clauses.b || []}
            selected={draft.b_clause}
            onSelect={(c) =>
              setField("b_clause", c)
            }
            testIdPrefix="b_clause"
          />

          <ClausePicker
            label="Conclusion (C)"
            clauses={clauses.c || []}
            selected={draft.c_clause}
            onSelect={(c) =>
              setField("c_clause", c)
            }
            testIdPrefix="c_clause"
          />
        </div>

        <div className="border-t border-line pt-4">
          <h3 className="text-xs tracking-[0.2em] uppercase text-ink-muted mb-2">
            Browse Conflicts
          </h3>
      ...
      <div className="flex-1 overflow-y-auto thin-scroll p-6 drafting-grid">
        ...
        {selectedHeading &&
          results.items.length > 0 && (
            <div
              className="mb-6"
              data-testid="plot-point-results"
            >
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-xs tracking-[0.2em] uppercase text-ink-muted">
                  {showAll
                    ? `All ${results.total} Plot Points`
                    : `Plot Points (${Math.min(
                        6,
                        results.total
                      )} of ${results.total})`}
                </h3>
        ...
        {(draft.a_clause ||
          ...
          draft.c_clause) && (
          <div
            className="mb-6 bg-surface border border-line-strong rounded-sm p-5"
            data-testid="skeleton-preview"
          >
            <h3 className="text-xs tracking-[0.2em] uppercase text-ink-muted mb-3">
              Masterplot
            </h3>
            ...
            <p className="text-sm text-ink leading-relaxed">
              ...
              {draft.b_clause && (
                <span>
                  {draft.b_clause.description}.{" "}
                </span>
              )}

              {draft.c_clause && (
                <span>
                  {draft.c_clause.description.replace(
                    /\.$/,
                    ""
                  )}
                  .
        ...
        <div data-testid="stepping-view">
          ...
          {draft.conflicts.length === 0 ? (
            ...
            <div className="space-y-3">
              ...
              {firstDetail &&
                firstDetail.lead_ups &&
                firstDetail.lead_ups.length > 0 &&
                firstDetail.lead_ups.some(
                  (g) => g.links.length > 0
                ) && (
                  <div data-testid="options-parent">
                    <div className="text-xs tracking-[0.15em] uppercase text-sage mb-2">
                      Story Past — add before beginning
                    </div>

                    {firstDetail.lead_ups.map(
                      (g, gi) =>
                        g.links.map(
                          (link, li) => {
                            const ordinal =
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-start gap-2">
                                  <span className="flex-shrink-0 w-5 h-5 flex items-center justify-center text-xs font-medium bg-sage/15 text-sage rounded-sm">
                                    {ordinal}
                                  </span>
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <ClauseText
                            text={summary}
                            className="text-sm text-ink leading-relaxed block"
                          />
                                ...
                                {detail.lead_ups
                                  ...
                                  .map(
                                    (m, mi) => (
                                      <span
                                        key={mi}
                                        className="text-xs px-2 py-0.5 bg-sage/10 text-sage border border-sage/20 rounded-sm"
                                      >
                                        {transformNote(
                                          m,
                                          characters
              ...
              {lastDetail &&
                lastDetail.carry_ons &&
                lastDetail.carry_ons.length >
                  0 &&
                lastDetail.carry_ons.some(
                  (g) => g.links.length > 0
                ) && (
                  <div data-testid="options-child">
                    <div className="text-xs tracking-[0.15em] uppercase text-sage mb-2">
                      Story Future — add after ending
                    </div>

                    {lastDetail.carry_ons.map(
                      (g, gi) =>
                        g.links.map(
                          (link, li) => {
                            const ordinal =
        ...
        {spineDetails.length > 0 && (
          <div
            className="mt-6 bg-surface border border-line-strong rounded-sm p-5"
            data-testid="final-presentation"
          >
            <h3 className="text-xs tracking-[0.2em] uppercase text-ink-muted mb-3">
              Your Story in Plain English
            </h3>

            {draft.a_clause && (
              <div
                className="mb-2"
                data-testid="present-beginning"
              >
            ...
            {draft.c_clause && (
              <div
                className="mb-2"
                data-testid="present-conclusion"
              >
                <span className="text-xs text-terra font-medium">
                  Conclusion ·{" "}
                </span>
            ...
            {!draft.c_clause && (
              <div
                className="mb-2"
                data-testid="present-no-conclusion"
              >
                <span className="text-xs text-ink-muted italic">
                  No conclusion clause selected yet.
                </span>
              </div>
            )}
          </div>
        ...
        {/* Notes */}
        <div className="mt-6">
          <label className="block text-xs tracking-[0.2em] uppercase text-ink-muted mb-2">
            Notes
          </label>
        ...
        {showSavePanel && (
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-heading uppercase tracking-tight text-ink">
                  Save Story
                </h3>
              ...
              {user ? (
                <>
                  ...
                  {allTexts.length > 0 && (
                    <div className="mt-4">
                      <label className="block text-xs tracking-[0.15em] uppercase text-ink-muted mb-2">
                        Cast
                      </label>
                ...
                <div className="text-center py-4">
                  <p className="text-sm text-ink-muted mb-4">
                    Sign in to save your story to your library.
                  </p>
